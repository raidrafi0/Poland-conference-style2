...
%	\end{table*}
	...
	$\ast$ \emph{States.} The states in the MDP are the views (images).  
	
	$\ast$  \emph{Rewards.} The reward $R$ takes a simple form, i.e., the correct tracking is considered as (+1), whereas, the reward $R$ of the incorrect tracking is considered as (-1). The correct and incorrect tracking are specified by comparing the regression layer outputs with the desired tracking codes. Measuring the successful process of the road tracking will be based on obtaining as many positive rewards as possible. 
		
	$\ast$ \emph{Policy search.} The DRL-RT network is based on the policy search. %According to the MDP concept, the 
